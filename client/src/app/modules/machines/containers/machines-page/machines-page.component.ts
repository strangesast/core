...
@Component({
  ...
  template: `<app-map-viewer></app-map-viewer>`,
})
  ...
  template: `
    ...
    <mat-table [dataSource]="dataSource" matSort>
      ...
      <ng-container matColumnDef="state">
        <mat-cell *matCellDef="let cell">
          {{ cell.state.length > 0 ? cell.state[0].value : null }}
        </mat-cell>
      </ng-container>
...
@Component({
  ...
  template: `
    <div class="grid">
      <a
        *ngFor="let each of data$ | async"
        [routerLink]="['/machines', each.machine_id]"
      >
        <span
          class="status"
          [ngClass]="
          ...
          "
          >{{ each.state.length > 0 ? each.state[0].value : 'unknown' }}</span
        >
